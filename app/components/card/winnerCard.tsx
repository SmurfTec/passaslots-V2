    ...
    <Card className="px-0" py={mediumdScreen ? 30 : 20} px={mediumdScreen ? 50 : 10} radius={20} bg="transparent">
      ...
      {data.slice(0, view).map((item, index) => (
        <div key={index}>
          <Group
            className={
              view === 10 && index === 9
                ? ' bg-[linear-gradient(0deg, #1A0E37 -61.35%, rgba(26, 14, 55, 0.49) 185.81%)] opacity-25'
                : ''
            }
            mt="lg"
            position="apart"
          >
            <Center className="items-start">
              ...
              <div className="space-y-1">
                <Text
                  color="white"
                  className="font-[600] leading-[18px] tracking-[0.0125em] text-[16px]"
                  size={mediumdScreen ? 'lg' : 'md'}
                >
                  {item.Username}
                </Text>
                <Text
                  color="white"
                  size={mediumdScreen ? 'sm' : 'xs'}
                  className="font-400 leading-[12px] tracking-[0.0125em] text-[11px]"
                >
                  {item.Game.length > 25 && !mediumdScreen ? item.Game.slice(0, 25) + '...' : item.Game}
                </Text>
              </div>
            </Center>
            <Center className="space-x-3">
              <Text
                color="white"
                size={mediumdScreen ? 16 : 12}
                className="text-[14px] leading-[15px] tracking-[0.0125em] font-[600]"
              >
                won
              </Text>
              <Text
                color="#50A1FF"
                size={mediumdScreen ? 25 : 20}
                className="text-[25px] leading-[27px] tracking-[0.0125em] font-[600]"
              >
                {item.Amount}
              </Text>
            </Center>
      ...
      <div className="text-center">
        <Text
          ...
          pt={60}
          pb={20}
          className="cursor-pointer font-[600] leading-[27px] tracking-[0.0125em] text-[25px] uppercase"
        >
